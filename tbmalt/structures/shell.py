...
This module provides the `Shell` data structure class and its associated code.
The `Shell` class is intended to hold data needed to describe the number and
...
class Shell:
    ...
    """
    # Developers Notes:
    # Shell instances only ever yield tensors used for masking or indexing
    ...
    def __init__(self, atomic_numbers: Union[List[Tensor], Tensor],
        ...
        if batch:
            def batch_reshape(t):
                ...
                return pack(torch.split(t, tuple(
    ...
    def device(self) -> torch.device:
        """The device on which the `Shell` object resides."""
        return self.__device

    @device.setter
    def device(self, *args):
        # Instruct users to use the ".to" method if wanting to change device.
        raise AttributeError('Shell object\'s dtype can only be modified '
                             'via the ".to" method.')
    ...
    def to(self, device: device) -> 'Shell':
        """Returns a copy of the `Shell` instance on the specified device.

        This method creates and returns a new copy of the `Shell` instance
        ...
        Returns:
            shell: Copy of the instance placed on the specified device.

        Notes:
            If the `Shell` instance is already on the desired device then
    ...
    def from_hdf5(cls, source: Group, device: Optional[torch.device] = None
        ...
        """Instantiate a `Shell` instances from an HDF5 group.

        Arguments:
            source: An HDF5 group(s) containing `Shell` instance.
            device: Device on which to place tensors. [DEFAULT=None]

        Returns:
            Shell: The resulting `Shell` object.
        """
    ...
    def to_hdf5(self, target: Group):
        """Saves `Shell` instance into a target HDF5 Group.

        Arguments:
            target: The hdf5 group to which the `Shell` should be saved.
    ...
    def azimuthal_matrix(self, form: Form = 'full', sort: bool = False,
        ...
        if form == 'full':
            ...
            if batch:
                basis_list = pack(torch.split(
                    basis_list, tuple(self.n_orbitals)), value=-1)
        else:
    ...
    def atomic_number_matrix(self, form: Form = 'full') -> Tensor:
        ...
        if form == 'full':
            ...
            if batch:
                an_mat = pack(torch.split(
                    an_mat, tuple(self.n_shells)), value=0)
    ...
    def index_matrix(self, form: Form = 'full') -> Tensor:
        ...
        if form == 'full':
            ...
            if self.atomic_numbers.dim() == 2:  # "if batch"
                i_mat = pack(torch.split(i_mat, tuple(self.n_shells)), value=-1)
