...
from tbmalt import Shell, Geometry, hs_matrix
...
class DftbGradient:
   ...
   """
    ...
    def _dftb1_grad(self, **kwargs):
        ...
        _shell = Shell(torch.repeat_interleave(
        ...
        _mask_u = self.geometry.distances.ne(
            0) * self.geometry.distances.triu().ne(0)
        _mask_l = self.geometry.distances.ne(
            0) * self.geometry.distances.tril().ne(0)
        ...
        ham0 = hs_matrix(self._geometry, _shell, self.h_feed)
        over0 = hs_matrix(self._geometry, _shell, self.s_feed)
        ...
        self._geometry.updated_dist_vec = dist_v.clone()
        ham1 = hs_matrix(self._geometry, _shell, self.h_feed)
        over1 = hs_matrix(self._geometry, _shell, self.s_feed)
        ...
        self.ham0_grad[..., mask[1], mask[0]] = - \
            self.ham0_grad[..., mask[1], mask[0]]
        self.over0_grad[..., mask[1], mask[0]] = - \
            self.over0_grad[..., mask[1], mask[0]]
    ...
    def _dftb1_grad2(self, **kwargs):
        ...
        # repeat three times to generate gradients over x, y, z directions
        _shell = Shell(self.geometry.atomic_numbers.repeat(3, 1),
        ...
        ham0 = hs_matrix(_geometry, _shell, self.h_feed)
        over0 = hs_matrix(_geometry, _shell, self.s_feed)
        ...
        _geometry.updated_dist_vec = dist_v.clone()
        ham1 = hs_matrix(_geometry, _shell, self.h_feed)
        over1 = hs_matrix(_geometry, _shell, self.s_feed)
        max_norb = ham1.shape[-1]
    ...
    def H0_grad(self):
        ...
        return orbs_to_atom((_h0_grad + _grad).sum(-1).reshape(-1, n_orb),
                            self.shell.orbs_per_atom.repeat(3, 1)).reshape(
    ...
    def repulsive_grad(self):
        ...
        atom_pairs = self.shell.atomic_number_matrix('atomic')
        ...
        for iap in uap:
            ...
            # 2. spline repulsive
            r_table = self.skparams.sktable_dict[(
                *iap.tolist(), 'spline_coef')]
            ...
            r_table_l = self.skparams.sktable_dict[(
                *iap.tolist(), 'tail_coef')]
    ...
    def gamma_grad(self, U: Tensor, distances: Tensor) -> Tensor:
        ...
        distances = distances.unsqueeze(
            0) if distances.dim() == 2 else distances
        ...
        r_homo, r_hetero = 1.0 / \
            distance[mask_homo], 1.0 / distance[mask_hetero]
